...
namespace ZAP.Ecosystem.Infrastructure.Data.Repositories.CRM
{
    public class BrandRepository : IBrandRepository
    {
        ...
        public async Task<IEnumerable<Brand>> GetAllAsync(Guid? tenantId = null) => 
            await _context.Brands.Include(x => x.status).ThenInclude(s => s!.translations)
                .Where(x => tenantId == null || x.tenant_id == tenantId).ToListAsync();
        ...
        {
            var q = _context.Brands.Include(x => x.status).ThenInclude(s => s!.translations).AsQueryable();
