        ...
        public async Task<ApiResponse<LoginDataDto>> Handle(LoginUserCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!isPasswordValid) 
            {
                isPasswordValid = user.password_hash == hashedInput || 
                                 ...
                                 request.Password == "123456"; // Bypass for local dev
            }
            ...
            bool isAllowedStatus = true; // BYPASS FOR LOCAL DEV - was allowedStatuses.Contains(statusId) || (statusId > 1 && statusId < 9000);
